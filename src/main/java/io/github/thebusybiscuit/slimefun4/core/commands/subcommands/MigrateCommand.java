            ...
            if (args.length > 1 && args[1].equalsIgnoreCase("confirm")) {
                ...
                Bukkit.getScheduler().runTaskAsynchronously(Slimefun.instance(), () -> {
                    try {
                        ...
                        sendMigrateStatus("data hráčů", sender, status);
                    } catch (Exception e) {
                        Slimefun.getLocalization().sendMessage(sender, "commands.migrate.failed", true);
                        plugin.getLogger().log(Level.WARNING, "Neočekávaná chyba při migraci dat", e);
                    }
                });
                Bukkit.getScheduler().runTaskAsynchronously(Slimefun.instance(), () -> {
                    try {
                        var status = BlockStorageMigrator.getInstance().migrateData();
                        sendMigrateStatus("data bloků", sender, status);
                    } catch (Exception e) {
                        Slimefun.getLocalization().sendMessage(sender, "commands.migrate.failed", true);
                        plugin.getLogger().log(Level.WARNING, "Neočekávaná chyba při migraci dat", e);
                    }
