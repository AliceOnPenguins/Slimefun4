    ...
    public void init() {
        ...
        try {
            ...
            if (blockStorageConfig.getBoolean("delayedWriting.enable")) {
                plugin.getLogger().log(Level.INFO, "Zpožděný zápis byl povolen");
            ...
            plugin.getLogger().log(Level.SEVERE, "Nepodařilo se načíst adaptér úložiště bloků Slimefunu", e);
            return;
        }
        ...
        try {
            ...
            plugin.getLogger().log(Level.SEVERE, "Nepodařilo se načíst adaptér profilů hráčů", e);
        }
    }
