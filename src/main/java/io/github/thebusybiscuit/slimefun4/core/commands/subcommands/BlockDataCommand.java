...
class BlockDataCommand extends SubCommand {
    ...
    public void onExecute(CommandSender sender, String[] args) {
        ...
        if (target == null || target.getType().isAir() || blockData == null) {
            ChatUtils.sendMessage(player, "&cMusíš se dívat na Slimefun blok, abys mohl použít tento příkaz!");
            return;
        }
        ...
        switch (args[1]) {
            case "get" -> {
                ...
                ChatUtils.sendMessage(
                        player,
                        "&aHodnota &b%key% &atohoto bloku je: &e%value%",
            ...
            case "set" -> {
                ...
                if (key.equalsIgnoreCase("id")) {
                    ChatUtils.sendMessage(player, "&cNemůžeš změnit ID bloku!");
                    return;
                }
                ...
                ChatUtils.sendMessage(
                        player,
                        "&aHodnota &b%key% &atohoto bloku byla nastavena na: &e%value%",
