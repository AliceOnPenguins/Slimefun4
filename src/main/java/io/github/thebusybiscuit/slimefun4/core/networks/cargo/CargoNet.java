...
public class CargoNet extends AbstractItemNetwork implements HologramOwner {
    ...
    public void tick(@Nonnull Block b, SlimefunBlockData blockData) {
        if (!regulator.equals(b.getLocation())) {
            updateHologram(b, "&4V okolí bylo nalezeno více regulátorů cargo sítě", blockData::isPendingRemove);
            return;
        }
        ...
        if (connectorNodes.isEmpty() && terminusNodes.isEmpty()) {
            updateHologram(b, "&cV okolí nebyl nalezen žádný uzel cargo sítě", blockData::isPendingRemove);
        } else {
            updateHologram(b, "&7Stav: &a&lPřipojeno", blockData::isPendingRemove);
