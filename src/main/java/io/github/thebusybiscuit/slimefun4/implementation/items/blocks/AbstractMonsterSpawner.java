...
public abstract class AbstractMonsterSpawner extends SlimefunItem implements DistinctiveItem {
    ...
    public Optional<EntityType> getEntityType(@Nonnull ItemStack item) {
        ...
        for (String line : meta.getLore()) {
            ...
            if ((stripColor.startsWith("Typ: ") || stripColor.startsWith("Type:"))
                    && (!line.contains("<Typ>") || line.contains("<Type>"))) {
    ...
    public ItemStack getItemForEntityType(@Nullable EntityType type) {
        ...
        for (int i = 0; i < lore.size(); i++) {
            ...
            if (currentLine.contains("<Type>") || currentLine.contains("<Typ>")) {
                String typeName = type == null ? "Prázdné" : ChatUtils.humanize(type.name());
                lore.set(i, currentLine.replace("<Type>", typeName).replace("<Typ>", typeName));
                break;
            }
        }
